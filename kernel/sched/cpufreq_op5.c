 ...
 * @sg_policy: op5 policy object to compute the new frequency for.
...
static void sugov_irq_work(struct irq_work *irq_work)
{
	...
	/*
	 * For RT and deadline tasks, the op5 governor shoots the
	 * frequency to maximum. Special care must be taken to ensure that this
	 * kthread doesn't result in the same behavior.
	 *
	 * This is (mostly) guaranteed by the work_in_progress flag. The flag is
	 * updated only at the end of the sugov_work() function and before that
	 * the op5 governor rejects all other frequency scaling requests.
	 *
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_OP5
static
#endif
struct cpufreq_governor cpufreq_gov_op5;
...
static int sugov_init(struct cpufreq_policy *policy)
{
	...
	if (policy->up_transition_delay_us && policy->down_transition_delay_us) {
		...
		unsigned int lat;
                ...
                tunables->up_rate_limit_us = 1;
                tunables->down_rate_limit_us = 3;
...
static int cpufreq_op5_cb(struct cpufreq_policy *policy,
				unsigned int event)
{
	switch(event) {
...
struct cpufreq_governor cpufreq_gov_op5 = {
	.name = "op5",
	.governor = cpufreq_op5_cb,
	.owner = THIS_MODULE,
};

static int __init sugov_register(void)
{
	return cpufreq_register_governor(&cpufreq_gov_op5);
}
